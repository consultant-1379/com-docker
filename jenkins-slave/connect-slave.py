...
# 1) Write slave.xml
# 2) wget http://com-docker-2.com.k2.ericsson.se:8080/jnlpJars/jenkins-cli.jar
# 3) java -jar jenkins-cli.jar -s http://com-docker-2.com.k2.ericsson.se:8080/ create-node slave-1 < slave.xml
# 4) wget http://com-docker-2.com.k2.ericsson.se:8080/jnlpJars/slave.jar
# 5) java -jar slave.jar -jnlpUrl http://com-docker-2.com.k2.ericsson.se:8080/computer/slave-1/slave-agent.jnlp
...
nodeName = label + "-" + hostname + "-" + uniqueString + "-" + os.environ['HOST']
...
if not preDownloadedJars:
    ...
    command = [ "wget", "-O", cliJarLocation, jenkinsCliUrl ]
...
if not preDownloadedJars:
    ...
    command = [ "wget", "-O", slaveJarLocation, jenkinsSlaveUrl ]
...
if not preDownloadedJars:
    jenkinsSlaveUrl = urlparse.urljoin(jenkinsUrl, "jnlpJars/agent.jar")
    command = [ "wget", "-O", agentJarLocation, jenkinsSlaveUrl ]
...
command = jenkinsAgentCommand + ["-jnlpUrl", jnlpUrl]
