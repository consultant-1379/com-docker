...
class BuildComMainCodeChecker(BuildBase):
    ...
    def _build(self):
        ...
        if self._args.baseCode:
            returnValue = call(["/usr/src/scripts/buildscripts/code_checker/scripts/parse.sh", "COM-MAIN", self._args.username, self._args.password, self._repoDir, commitHash, self._args.baseCode], debug=True)
        else:
            returnValue = call(["/usr/src/scripts/buildscripts/code_checker/scripts/parse.sh", "COM-MAIN", self._args.username, self._args.password, self._repoDir, commitHash], debug=True)
        if returnValue > 0:
