...
class Gask(StartBase):
    ...
    def __init__(self):
        ...
        self._imageName = "armdocker.rnd.ericsson.se/cba-com/gask-upload:latest"
    ...
    def _gaskUpload(self, params):
        ...
        if self._imageName is not None:
            ...
            try:
                # add constraint to run only on gask nodes
                extraRunParams = ["-e", "constraint:type==functiontest"]
                try:
                    gaskComponentList = os.environ['componentList']
                    if gaskComponentList is not None:
                        extraRunParams += ["-e", "componentList="+gaskComponentList.lstrip()]
                except KeyError:
                    raise Exception('Error: Component list to perform gask is not set')
    ...
    def execute(self, args):
        ...
        print("Starting the Gask upload")
        return self._gaskUpload(["gaskupload",
                                  args["username"],
                                  args["password"],
                                  args["comVersion"],
                                  ])
